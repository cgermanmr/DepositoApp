...
namespace DAL
{
    public class ActivoDAL : Repositorio
    {
        ...
        protected override Hashtable ObtenerParametros(Entidad valor)
        {
            ...
            hdatos.Add("@usuario", _dato.UsuarioModificador);
        ...
        private List<ActivoBEL> ObtenerListaH(DataSet ds)
        {
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                x = new ActivoBEL();
                x.Id = (int)dr[1];
                x.FechaAlta = (DateTime)dr[2];
                x.Inventario = Convert.IsDBNull(dr[3]) ? string.Empty : dr[3].ToString();
                x.Serie = Convert.IsDBNull(dr[4]) ? string.Empty : dr[4].ToString();
                x.Descripcion = Convert.IsDBNull(dr[5]) ? string.Empty : dr[5].ToString();
                x.Sector = (SectorBEL)new SectorDAL().GetById(dr[6]);
                x.Ubicacion = (UbicacionBEL)new UbicacionDAL().GetById(dr[7]);
                x.EstadoActivo = (EstadoBEL)new EstadoDAL().GetById(dr[8]);
                x.TipoActivo = new TipoActivoDAL().GetById(dr[9]) as TipoActivoBEL;
                x.Marca = (MarcaBEL)new MarcaDAL().GetById(dr[10]);
                x.Organizacion = new EmpresaDAL().GetById(dr[11]) as EmpresaBEL;
                x.Moneda = (MonedaBEL)new MonedaDAL().GetById(dr[12]);
                x.ValorCompra = Convert.IsDBNull(dr[13]) ? 0.0 : (double)dr[13];
                x.ValorSoporte = Convert.IsDBNull(dr[14]) ? 0.0 : (double)dr[14];
                x.FechaCompra = dr[15].SafeToDateTime();
                x.MesesGarantia = Convert.IsDBNull(dr[16]) ? 0 : Convert.ToInt32(dr[16]);
                x.Volumen = Convert.IsDBNull(dr[17]) ? 0 : (int)dr[17];
                x.Observaciones = Convert.IsDBNull(dr[18]) ? string.Empty : (string)dr[18];
                x.UsuarioModificador = dr[19].ToString();
                x.FechaModificacion = dr[20].SafeToDateTime();
                x.TipoModificacion = (TipoOperacion)dr[21].SafeToInt();
                _lista.Add(x);
            }
            return _lista.OrderByDescending(a => a.FechaModificacion).ToList();
        }
    }
}
